...
export function generateCollectEmbedScript(
  ...
  const panelPosCSS = isPopup
    ...
    : "bottom:90px;" + posCSS;
  const panelClosedTransform = isPopup
    ? "translate(-50%,-46%) scale(0.96)"
    : w.position === "bottom-center"
    ? "translateX(-50%) translateY(20px)"
    : "translateY(20px)";
  const panelOpenTransform = isPopup
...
var $=isInline ? function(id){return c.querySelector('#'+id)} : function(id){return sh.getElementById(id)};
var posCSS='${posCSS}';
sh.innerHTML='<style>'
...
+'.wp-panel{font-family:"Inter",-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;'
+(isInline ? 'position:static;width:100%;max-width:560px;' : 'position:absolute;'+panelPosCSS+';width:'+(isPopup ? 'min(500px, calc(100vw - 32px))' : 'min(420px, calc(100vw - 32px))')+';max-width:calc(100vw - 32px);')
+'max-height:'+(isInline ? 'none' : 'min(680px, calc(100vh - 32px))')+';max-height:'+(isInline ? 'none' : 'min(680px, calc(100dvh - 32px))')+';'
+'background:#fff;border-radius:20px;box-shadow:0 25px 50px -12px rgba(0,0,0,0.25),0 0 0 1px rgba(0,0,0,0.06);overflow:hidden;'
+(isInline ? 'opacity:1;transform:none;pointer-events:auto;' : 'transform:'+panelClosedTransform+';opacity:0;transition:all 0.35s cubic-bezier(0.16,1,0.3,1);pointer-events:none;')
+'display:flex;flex-direction:column}'
+'.wp-panel.open{opacity:1;pointer-events:all;transform:'+panelOpenTransform+'}'
...
+'.wp-hdr{display:flex;align-items:flex-start;justify-content:space-between;padding:24px 24px 0;flex-shrink:0;gap:12px}'
...
+'.wp-close{width:32px;height:32px;border-radius:10px;border:1px solid #e5e7eb;background:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#6b7280;transition:all 0.15s;flex-shrink:0;margin-top:2px;touch-action:manipulation}'
+'.wp-close:hover{background:#f3f4f6;color:#111827;border-color:#d1d5db}'
+'.wp-desc{display:none}'
+'.wp-form{padding:20px 24px 24px;overflow-y:auto;flex:1 1 auto;display:flex;flex-direction:column;gap:14px;-webkit-overflow-scrolling:touch;overscroll-behavior:contain}'
+'.wp-stars{display:flex;gap:8px;font-size:32px;cursor:pointer;justify-content:center;padding:4px 0;touch-action:manipulation}'
+'.wp-stars span{transition:transform 0.15s ease,filter 0.15s ease;filter:grayscale(1) opacity(0.25);cursor:pointer;user-select:none;display:flex;align-items:center;justify-content:center;min-width:40px;min-height:40px}'
...
+'.wp-field-group{display:flex;align-items:center;gap:12px;background:#fff;border:1.5px solid #e5e7eb;border-radius:14px;padding:0 16px;transition:all 0.25s cubic-bezier(0.4,0,0.2,1);box-shadow:0 1px 2px rgba(0,0,0,0.04);flex-shrink:0}'
...
+'.wp-textarea-wrap{display:flex;flex-direction:column;gap:6px;flex-shrink:0}'
+'.wp-char{font-size:12px;color:#9ca3af;text-align:right;font-weight:500;padding-right:4px}'
+'.wp-submit{width:100%;min-height:48px;padding:14px;border:none;border-radius:12px;background:'+W+';color:#fff;font-size:15px;font-weight:600;cursor:pointer;transition:all 0.2s;letter-spacing:-0.01em;margin-top:4px;flex-shrink:0;touch-action:manipulation}'
...
+'.wp-backdrop{position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:99998;opacity:0;transition:opacity 0.3s ease;pointer-events:none;backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px)}'
+'.wp-backdrop.open{opacity:1;pointer-events:auto}'
+'.wp-succ{display:none;padding:40px 24px;text-align:center;flex:1 1 auto;overflow-y:auto}'
...
+'.wp-err{color:#ef4444;font-size:13px;text-align:center;display:none;padding:8px 12px;background:#fef2f2;border-radius:8px;font-weight:500;flex-shrink:0}'
+'.wp-ft{padding:14px 24px;border-top:1px solid #f3f4f6;text-align:center;font-size:12px;color:#9ca3af;flex-shrink:0}'
...
+'.wp-panel{width:calc(100vw - 24px) !important;max-width:calc(100vw - 24px) !important;max-height:calc(100vh - 24px) !important;max-height:calc(100dvh - 24px) !important;border-radius:16px !important}'
+'.wp-hdr{padding:18px 18px 0 !important}'
+'.wp-hdr h3{font-size:18px !important}'
+'.wp-subtitle{font-size:12px !important}'
...
+'<div class="wp-hdr"><div><h3>'+H+'</h3><p class="wp-subtitle">'+D+'</p></div><button class="wp-close" id="cb" aria-label="Close modal"><svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="1" y1="1" x2="13" y2="13"/><line x1="13" y1="1" x2="1" y2="13"/></svg></button></div>'
+'<form class="wp-form" id="fm">'
+(S?'<div class="wp-stars" id="st">'+Array(5).fill(0).map((_,i)=>'<span data-i="'+i+'" role="button" aria-label="Rate '+(i+1)+' stars"><svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></span>').join('')+'</div>':'')
...
+'<div class="wp-ft"><span id="pw" style="display:'+(PB?'block':'none')+'"><a href="https://wallproud.com" target="_blank" rel="noreferrer">Powered by WallProud</a></span></div>'
+'</div>'
...
tb.id='tb';
tb.setAttribute('aria-label', 'Open feedback form');
tb.textContent='💬';
var btnPos='position:fixed;bottom:24px;${posCSS};width:56px;height:56px;border-radius:50%;background:${w.primary_color};color:#fff;border:none;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,0.2);font-size:24px;z-index:99999;display:flex;align-items:center;justify-content:center;transition:transform 0.2s;pointer-events:auto;touch-action:manipulation';
...
bd.className='wp-backdrop';
document.body.appendChild(bd);
var closeModal=function(){pn.classList.remove('open');bd.classList.remove('open')};
bd.addEventListener('click',closeModal);
bd.addEventListener('touchstart',closeModal,{passive:true});
` : ''}
...
document.addEventListener('keydown',function(e){
  if(e.key==='Escape' && pn.classList.contains('open') && DT !== 'inline'){
    pn.classList.remove('open');
    ${isPopup ? 'bd.classList.remove("open");' : ''}
  }
});
...
if(sp>=pct&&!sd){sd=true;pn.classList.add('open');if(isPopup&&bd)bd.classList.add('open');window.removeEventListener('scroll',fn)}
});
}else if(TR==='exit-intent'){
document.addEventListener('mouseleave',function(e){if(e.clientY<=0){pn.classList.add('open');if(isPopup&&bd)bd.classList.add('open')}});
}else if(TR==='timed'){
setTimeout(function(){pn.classList.add('open');if(isPopup&&bd)bd.classList.add('open')},(DELAY_S||5)*1000);
}
...
` : ''}
setTimeout(function(){if(DT !== 'inline'){pn.classList.remove('open')${isPopup ? ';if(bd)bd.classList.remove("open")' : ''}}},AC*1000);
